    ...
    m_thread = std::make_unique<std::thread>([this]() {
        ...
        ([this](const crow::request& req, std::string dataset_id) {
            try {
                ...
                // If the job completed synchronously, include result_summary and result_plot
                // in the POST response so the frontend doesn't need to poll.
                auto snap = m_job_store.snapshot(job_id);
                if (snap["analysis_type"] == "downsample") {
                    snap["interactive_kind"] = "downsample";
                }
                if (snap["status"] == "completed" && snap.contains("result_dataset_id") && !snap["result_dataset_id"].is_null()) {
                    ...
                    try {
                        ...
                        if (snap["analysis_type"] == "downsample") {
                            snap["result_info"]["interactive_kind"] = "downsample";
                        }
                    } catch (...) {}
                }
